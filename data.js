const siteData = {
  "header": {
    "catchphrase": "〜いま、ここ、こそ、パラダイス〜"
  },
  "topics": {
    "media": "パラレルドリーム・ランデブー_SNS_movie.mp4",
    "comment": "「第六回公演」今秋決定！",
    "link": "#performance"
  },
  "about": {
    "title": "「劇団ハレトケ」とは",
    "catchphrase": "〜いま、ここ、こそ、パラダイス〜",
    "paragraphs": [
      "劇団ハレトケは、「いま、ここ、こそ、パラダイス」をスローガンに掲げ、関東を中心に活動するミュージカル団体です。",
      "忙しい毎日の中で、私たちはつい自分自身と向き合う時間を見失いがちです。",
      "劇団ハレトケは、演劇やミュージカルを通して、自分らしさや心の動きに目を向けるきっかけを生み出したいと考えています。",
      "団体名の「ハレトケ」は、民俗学における考え方「ハレ（非日常）」と「ケ（日常）」に由来しています。",
      "私たちは、日常の中にほんの少しの特別な時間を届けることで、観る人にも演じる人にも心が豊かになる体験を創りたいと願っています。",
      "上演する作品はすべて団員によるオリジナル作品です。脚本をはじめ、作詞・作曲、演出はもちろん、音響・照明・小道具・衣装などのスタッフワークも団員自らが担当し、試行錯誤を重ねながら一つひとつの作品を創り上げています。",
      "舞台が終わったあとも、その時間や感情が日々の暮らしを少しだけ彩ってくれる。",
      "そんな「日常と非日常をつなぐ場所」であり続けることを目指しています。"
    ],
    "logoImage": "gekidan haretoke_logo_icon (1).jpg",
    "logoParagraphs": [
      "ロゴには「ハレ」と「ケ」を舞台のセンターバミリ「ト」で繋ぐ様を表現しています\nダミーダミーダミーダミーダミーダミーダミー\nダミーダミーダミーダミーダミーダミーダミーダミーダミーダミーダミーダミーダミーダミー\nダミーダミーダミー",
      "ダミーダミーダミーダミーダミーダミーダミー\nダミーダミーダミーダミーダミーダミーダミーダミーダミーダミーダミーダミーダミーダミー\nダミーダミーダミーダミーダミーダミーダミー"
    ]
  },
  "members": {
    "title": "「劇団ハレトケ」",
    "countText": "団員 〇〇名 (2026/05現在)",
    "membersList": [
      { "name": "ごっちゃん" },
      { "name": "みなや" },
      { "name": "がく" },
      { "name": "監物" },
      { "name": "いきちゃん" },
      { "name": "〇〇〇〇〇" },
      { "name": "〇〇〇〇〇" },
      { "name": "〇〇〇〇〇" },
      { "name": "〇〇〇〇〇" },
      { "name": "〇〇〇〇〇" },
      { "name": "〇〇〇〇〇" },
      { "name": "〇〇〇〇〇" }
    ]
  },
  "stage": {
    "title": "STAGE",
    "subtitle": "第6回公演「パラレルドリーム・ランデブー」",
    "description": "次回公演に向けて鋭意稽古中です！\nティザー動画もぜひご覧ください。",
    "staff": "脚本・演出: 秋梨夜風",
    "date": "",
    "venue": "JOY JOY THEATER (JOY JOY STATION内)",
    "note": "",
    "video": "",
    "images": [
      "パラレルドリーム・ランデブー_logokv_0726.jpg",
      "パラレルドリーム・ランデブー_introduction.webp",
      "パラレルドリーム・ランデブー_cast.webp"
    ],
    "ticketUrl": "ticket.html",
    "ticketComment": ""
  },
  "memory": {
    "title": "MEMORY",
    "performances": [
      {
        "id": "perf6",
        "title": "第5回公演「怪盗スターダスト」",
        "description": "あらすじや、公演に込めた思いなどを記載します。",
        "staff": "脚本・演出: ゆい",
        "date": "202X年 X月",
        "venue": "〇〇劇場",
        "image": "Stardust_mv.JPG"
      },
      {
        "id": "perf5",
        "title": "第4回公演「Terreur」",
        "description": "あらすじや、公演に込めた思いなどを記載します。",
        "staff": "脚本・演出: ",
        "date": "202X年 X月",
        "venue": "〇〇劇場",
        "image": "Terreur.JPG"
      },
      {
        "id": "perf4",
        "title": "グリーンカレッジ公演「注文の多い音楽会」",
        "description": "あらすじや、公演に込めた思いなどを記載します。",
        "staff": "脚本・演出: ゆい",
        "date": "202X年 X月",
        "venue": "〇〇劇場",
        "image": "Chumonnoooi.PNG"
      },
      {
        "id": "perf3",
        "title": "第3回公演「オールド・サマー・バケーション」",
        "description": "あらすじや、公演に込めた思いなどを記載します。",
        "staff": "脚本・演出: 秋梨夜風",
        "date": "202X年 X月",
        "venue": "〇〇劇場",
        "image": "OSV_mv_0814.jpg"
      },
      {
        "id": "perf2",
        "title": "第2回公演「ゴースト・アクター」",
        "description": "あらすじや、公演に込めた思いなどを記載します。",
        "staff": "脚本・演出: ゆい",
        "date": "202X年 X月",
        "venue": "〇〇劇場",
        "image": "GA_mv_A3_0109_big.jpg"
      },
      {
        "id": "perf1",
        "title": "旗揚げ公演『ハレ舞台』",
        "description": "自分が今、目の当たりにしているものが現実なのか夢なのか想像なのか、よく分からなくなってしまう事ってありませんか？\nどれであるにせよ、素敵なあなたの物語に変わりありません。\nハートフルな作品ばかりじゃない、辛いものや残酷なものも時にはあるはずです。しかし、結末を急ぎすぎてはなりません。\n物語を織りなす人物として、一世一代のハレ舞台を大切にすごそうじゃありませんか。\nもっとあなたらしい物語が出来上がるはずです。",
        "staff": "脚本・演出: みなや",
        "date": "2023年 8月20,21日",
        "venue": "JOY JOY THEATER",
        "image": "harebutai_MV_for-SNS.jpg"
      }
    ]
  },
  "links": {
    "title": "団員募集について",
    "recruitment": [
      "🎭✨新メンバー大募集✨🎭<br>歌って！踊って！演じてみたいそこのあなた！<br>私たちと一緒にミュージカルをはじめてみませんか？",
      "＼こんな人、大歓迎！／<br>🎤歌うのが好き！<br>💃ダンスが楽しい！<br>🎭お芝居やってみたい！<br>🎨小道具・衣装づくりが好き！<br>🎼曲を作ってみたい！<br>📝脚本書いてみたい！<br>舞台の上も、裏方も、みんな主役🌟<br>初心者・未経験でもOK！"
    ],
    "fee": "【会費】<br>団体費:上期下期各1,000円<br>別途公演参加費:10,000円程度<br>※公演参加は必須ではありません<br>公演に役者として関わる場合にのみ発生",
    "linkText": "「劇団員になろうよ」募集ページ",
    "linkUrl": "https://theatrical.net-menber.com/look/data/216338.html?utm_source=ig&utm_medium=social&utm_content=link_in_bio&fbclid=PAdGRleARheVBleHRuA2FlbQIxMQBzcnRjBmFwcF9pZA8xMjQwMjQ1NzQyODc0MTQAAadUoPp6z35Lg9kmNLe7wWCzOyfvG808flBKRdYHTxTvCdojMudfaVHRfsGx2g_aem_yfhOUztngHnuBaVnc2Kjn"
  },
  "contact": {
    "title": "お問い合わせ / 各種SNS",
    "description1": "公演に関するご質問や、チケットのお問い合わせは以下のフォームより承っております。",
    "formText": "お問い合わせフォームはこちら",
    "formUrl": "https://docs.google.com/forms/d/e/1FAIpQLSelOIThFwEVI8Yc99L6PChKt-mCClAt8G852VLiGTJseDqEqg/viewform?usp=header",
    "snsTitle": "公式SNSアカウント",
    "twitterId": "@haretokemusical",
    "twitterUrl": "https://x.com/haretokemusical?s=21&t=mtbf5MTQ_Et1zKMI2koXbw",
    "instagramId": "@haretoke.musical",
    "instagramUrl": "https://www.instagram.com/haretoke.musical?igsh=bDhucXAyNXgzcXB3&utm_source=qr",
    "youtubeId": "劇団ハレトケ<br>YouTube",
    "youtubeUrl": "https://youtube.com/channel/UCKKov_LWYETl3ohX0XLZ2FA?si=jWwnxhVn3OdI5ATY"
  }
};
